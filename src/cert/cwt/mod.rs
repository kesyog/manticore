...
pub fn parse<'cert>(
    ...
    verifier
        .verify(
            &[
                // Length prefix for the whole array: major 0b100, length 4;
                // length prefix for the string: major 0b011, length 10.
                b"\x84\x6aSignature1",
                // Encoded protected bucket map.
                cose.protected_bytes.as_slice_less_safe(),
...
impl<'cert> Cose<'cert> {
    fn parse(buf: &mut untrusted::Reader<'cert>) -> Result<Cose<'cert>, Error> {
        ...
        let (protected_bytes, Headers { algo, kid }) =
            buf.read_partial(|buf| {
                Item::parse(buf)?.read_all(|buf| {
